...
import { updateCostingRecipe } from '@/actions/costing';
...
export default function EditRecipeModal({ recipeId, isOpen, onClose }: EditRecipeModalProps) {
  const { recipe, isLoading: isLoadingRecipe } = useCostingRecipe(recipeId);
  ...
  const updateRecipe = useCostingRecipesStore((state) => state.updateRecipe);

  // Load recipe data when it's available
  useEffect(() => {
    if (recipe) {
      setFormData({
        recipe_name: recipe.recipe_name,
        servings: recipe.servings?.toString() || '',
        sell_price: recipe.sell_price?.toString() || '',
      });
      setIngredients(
        recipe.ingredients.map((ing) => ({
          ...
          unit: ing.unit,
          price_per_unit: ing.price_per_unit_snapshot,
        }))
      );
    }
  }, [recipe]);
  ...
  const calculateTotalCost = () => {
    return ingredients.reduce((total, ing) => {
      ...
      return total + quantity * (masterIng?.price_per_unit || 0);
    }, 0);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await updateCostingRecipe(recipeId, {
      ...
      if (result.success && result.data) {
        updateRecipe(recipeId, result.data);
        onClose();
      } else {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-gray-900">Edit Food Cost Recipe</h2>
          <button
            onClick={handleClose}
                      ...
                      >
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-gray-500">
                            ${(masterIngredients.find(m => m.id === ing.ingredient_id)?.price_per_unit || 0).toFixed(4)}/{ing.unit}
                          </div>
                        </div>
                        <div className="w-28">
                          <input
                        ...
                        <div className="w-20 text-right">
                          <div className="text-sm font-semibold text-gray-900">
                            ${((parseFloat(ing.quantity) || 0) * (masterIngredients.find(m => m.id === ing.ingredient_id)?.price_per_unit || 0)).toFixed(6)}
                          </div>
                        </div>
                        <button
                          type="button"
